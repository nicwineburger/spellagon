...
## Puzzles
Each day's puzzle is the original's, fetched once a day by the `puzzles` workflow into `public/puzzles/`. The site loads those files from its own origin, so your browser never contacts anyone else. A day without one falls back to our own puzzle.

Our word list comes from [SCOWL](http://wordlist.aspell.net/) through `wordlist-english`, filtered to common words with no S, and with a blocklist for swearing and slurs.
